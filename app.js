...
const lastEntrySection = document.getElementById('lastEntry');
const lastEntryDesc = document.getElementById('lastEntryDesc');
const lastEntryMeta = document.getElementById('lastEntryMeta');
const lastEntryValor = document.getElementById('lastEntryValor');
...
/* -----------------------------------------------------------
   Último lançamento — persistido neste dispositivo
   ----------------------------------------------------------- */
const LAST_ENTRY_KEY = 'lancamentos-last-entry';
...
function renderLastEntry(entry) {
  if (!entry) {
    lastEntrySection.hidden = true;
    return;
  }

  lastEntryDesc.textContent = entry.descricao;
  lastEntryMeta.textContent = `${entry.categoria} · ${entry.pessoa} · ${formatDateBR(entry.data)}`;

  const sinal = entry.tipo === 'Receita' ? '+ ' : '− ';
  lastEntryValor.textContent = sinal + formatValorBRL(entry.valor);
  lastEntryValor.className = 'last-entry-valor ' + (entry.tipo === 'Receita' ? 'tipo-receita' : 'tipo-despesa');

  lastEntrySection.hidden = false;
}
...
function saveLastEntry(entry) {
  ...
  renderLastEntry(entry);
}

function loadLastEntry() {
  try {
    const raw = localStorage.getItem(LAST_ENTRY_KEY);
    if (raw) renderLastEntry(JSON.parse(raw));
  } catch (err) {
    /* dado corrompido ou indisponível: ignora */
  }
}
